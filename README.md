# YOLOv11 Platform — 一站式 YOLO 訓練與測試平台
...
- **資料集管理**：用原生資料夾選擇視窗（或手動貼路徑）指定本機資料夾，不複製影像；
  可選 train/val 自動切分或同一資料夾；同名覆蓋會先確認
- **模型訓練**：訓練模式套餐（快速試跑／標準／精細／自訂）一鍵填參數，常用超參數一級控制 +
  進階 JSON 透傳；自動偵測並使用 GPU；AutoBatch 避免記憶體不足；同時只跑一個訓練以防 OOM
- **訓練監控**：即時 mAP/loss 曲線、紅綠燈判讀、本次參數、**訓練圖表**（混淆矩陣／PR 曲線／
  預測樣本）；**可隨時取消**（當前 epoch 結束後停止、保留權重）與**繼續訓練**；完成有通知；
  紀錄落地保存、**後端重啟後不消失**，手動拷入 `runs/` 的舊訓練也會自動列出；任務可刪除
- **推論測試**：資料夾瀏覽模式，逐張或整批推論，難例一鍵複製到 `need_to_train/`，標註圖可打包下載
- **模型庫 / 管理**：列出已訓練模型與 mAP 判讀；刪除資料集（不碰來源影像）、刪除訓練成果、
  管理 `need_to_train`（自選或清空）；側邊欄顯示磁碟用量與剩餘空間
